	...
	async up(queryInterface, Sequelize) {
        ...
        COUNT(
			...
			await queryInterface.bulkUpdate(
				'report_queries',
				{
                    ...
                    FROM
                        Session
                    LEFT JOIN
                        UserExtensions AS ue ON Session.created_by = ue.user_id AND Session.tenant_code = ue.tenant_code
                    JOIN
			...
			await queryInterface.bulkUpdate(
                ...
                WHERE
                    session.tenant_code = :tenantCode
                    AND (:userId IS NOT NULL AND so.user_id = :userId OR :userId IS NULL)
                    AND session.deleted_at IS NULL
                    DYNAMIC_AND_CLAUSE;`,
					updated_at: Sequelize.literal('CURRENT_TIMESTAMP'),
				},
			...
			await queryInterface.bulkUpdate(
				'report_queries',
				{
                    ...
                    SELECT 
                    session.tenant_code,
                    session.mentor_id, session.mentor_name, COUNT(*) AS number_of_sessions,
                    ...
                    FROM public.sessions AS session WHERE 
                    session.tenant_code = :tenantCode AND deleted_at IS NULL AND session.created_by = :userId AND session.started_at IS NOT NULL AND session.completed_at IS NOT NULL AND session.start_date > :start_date AND session.end_date < :end_date AND (CASE
                                            WHEN :session_type = 'All' THEN session.type IN ('PUBLIC', 'PRIVATE')
                                            WHEN :session_type = 'Public' THEN session.type = 'PUBLIC'
                                            WHEN :session_type = 'Private' THEN session.type = 'PRIVATE'
                                            ELSE TRUE
