...
module.exports = class ConnectionHelper {
	 ...
	 */
	static async checkConnectionIfExists(user_id, body, tenant_code) {
		try {
			...
			const userInfo = await communicationHelper.resolve(friend_id, tenant_code)
			if (!userInfo) {
				return responses.failureResponse({
					responseCode: 'CLIENT_ERROR',
					statusCode: httpStatusCode.not_found,
			...
			const connectionCheck = await connectionQueries.getConnection(user_id, userInfo.user_id, tenant_code)
